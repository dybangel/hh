	...
	$.fn.Refresh = function(options)
	{
			...
			SendOptions['success'] = function(strJson)
			{
				try{
					if (downOrUp == 'down') 
					{
						...
						try{mescroll.endSuccess(FormatPager,true);}catch(err){}
					}
					else
					{
						...
						if(strJson!="")
						{
							try{mescroll.endBySize(FormatPager,true);}catch(err){}
						}else{
							try{mescroll.endBySize(FormatPager,false);}catch(err){}
						}
					}
				}catch(err){}
			};
		...
		********************************************************************/
		...
		&& options['url']!=null && options['url']!='')
		{
			...
			window.scroller = new Scroller(contianer, scrollerConfig);  
		};
	};
})(jQuery);
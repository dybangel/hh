...
"use strict";
	$.fn.eKaoshi = function(options,arrList)
	{
				...
				&& thisAnswer!=undefined && thisAnswer!=null && thisAnswer=="")
				{
					...
					**************************************************************/
					$(frmTemplate).find("#autoTrue").click(function(){
						...
						&& GetPlayerValue()!='关闭语音' && Operate['thisOption']['thisAnswer']=="")
						{
							try{VerificationSelectionJudge("对");}
							catch(err){}
						}
					});	
					/*************************************************************
					...
					**************************************************************/
					$(frmTemplate).find("#autoFalse").click(function(){
						...
						&& GetPlayerValue()!='关闭语音' && Operate['thisOption']['thisAnswer']=="")
						{
							try{VerificationSelectionJudge("错");}
							catch(err){}
						}
					});	
				};
			...
			try{
				...
				*************************************************************************************/
				try{
					...
					var gotoHeight = parseInt(((rowsIndex-1)*40));
		...
		var StartInterval = function(){
			try{
				if(Operate['timer']!=undefined && Operate['timer']!=null){
					try{clearInterval(Operate['timer']);}catch(err){}
				};
				Operate['timer'] = setInterval(function(){
					...
					else
					{
						...
						try{Operate['isTimeup'] = true;}catch(err){}
						try{SubmitForceKaoshi(true);}catch(err){}	
					}
				},1000);	
			}catch(err){}
		};
		...
		*弹出确认提示按钮信息
		*********************************************************************************************/
		var thisConfirm = function(tips,okSuccess)
		{
			try{
				$("#frmAssContext").html(tips);
				$("#frmAssMaster").show();
			}catch(err){}
		};
		/********************************************************************************************
		*确认交卷信息
		*********************************************************************************************/
		var SubmitKaoshi = function(){
			try{
				var SendOption = {};
				try{
					SendOption["url"]="Kaoshi.aspx?action=submit";
					SendOption['type']="post";
					SendOption['dataType']="json";
					SendOption["data"] = {
						"strContext":JSON.stringify(arrList['items']),
					...
					{
						var options = {};
						try{SubmitHandleResponse(strResponse);}
						catch(err){}
					};
		...
		*********************************************************************************************/
		var render = function()
		{
			...
			*选项栏以及图片栏
			****************************************************************************************/
			strTemplate += "<div id=\"frmInterMaster\">";
			strTemplate += "<div id=\"frmInterbox\">";
			strTemplate += "<span id=\"frmIntertext\">倒计时</span>";
			strTemplate += "<span id=\"frmInterval\">45:00:00</span>";
			strTemplate += "</div>";
			strTemplate += "</div>";
			...
			****************************************************************************************/
			...
			strTemplate += "<div value=\"交卷\" id=\"FrmAssignment\"></div>";
			...
			*提示正在加载信息
			***********************************************************************************/
			strTemplate += "<div title=\"正在加载弹出框\" id=\"frmLoadMaster\">";
			...
			*提交试卷弹出框
			***********************************************************************************/
			strTemplate += "<div title=\"提交试卷弹出框\" id=\"frmAssMaster\">";
			strTemplate += "<div id=\"frmAssContianer\">";
			strTemplate += "<div id=\"frmAssTitle\">温馨提示</div>";
			strTemplate += "<div id=\"frmAssContext\"></div>";
			strTemplate += "<div id=\"frmAssBtns\">";
			strTemplate += "<div id=\"frmAssOK\">确定交卷</div><div id=\"frmAssCancel\">取消</div>";
			strTemplate += "</div>"
			strTemplate += "</div>";
			strTemplate += "</div>";
			...
			*弹出强制交卷提示信息
			***********************************************************************************/
			strTemplate += "<div title=\"强制交卷提示信息\" id=\"ShowSubmitMasker\">";
			strTemplate += "<div id=\"ShowSubmitContianer\">考试答题时间已到,系统将在6后自动交卷</div>";
			strTemplate += "</div>";
			/**********************************************************************************
			*显示答题卡选项信息
			***********************************************************************************/
			...
			****************************************************************************************/
			...
			Operate['FrmAssignment'] = document.querySelector("#FrmAssignment") || null;
			...
			****************************************************************************************/
			...
			Operate['ShowSubmitContianer'] = document.querySelector("#ShowSubmitContianer") || null;
						...
						if(GetPlayerValue()!='语音讲解'){
							...
							var inter = setInterval(function(){
								if(timer>=0){timer=timer-1;}
									...
									try{
										clearInterval(inter);
										if(GetPlayerValue()!='语音讲解'){ShowPlayer();}	
									}catch(err){}
								}
							},500);
						}
			...
			*点击交卷按钮
			****************************************************************************************/
			if(Operate['FrmAssignment']!=undefined && Operate['FrmAssignment']!=null)
			{
				try{
					$(Operate['FrmAssignment']).click(function(){
						try{
							var UnderNumber = 0;var TrueNumber = 0; var FalseNumber = 0;
							var TotalNumber = parseInt(arrList['items'].length) || 0;
							if(TotalNumber<=0){TotalNumber=1;}
							$(arrList['items']).each(function(k,Json){
								if(Json['isTrue']=="1"){TrueNumber=TrueNumber+1;}
								else if(Json['isTrue']=="0"){FalseNumber=FalseNumber+1;}
								else if(Json['isTrue']=="-1"){UnderNumber=UnderNumber+1;}
							});
							var ResNumber = ((100 / TotalNumber) * TrueNumber).toFixed(0);
							var strTips = "你目前答对了<font style=\"color:#cd0000;margin:0px 5px;\">"+TrueNumber+"</font>道试题,";
							strTips +="还有<font style=\"color:#cd0000;margin:0px 5px;\">"+UnderNumber+"</font>道试题未回答,";
							strTips +="考试得分为<font style=\"color:#cd0000;margin:0px 5px;\">"+ResNumber+"</font>分,你确定要交卷吗?"
							WindowsConfirm(strTips,function(){SubmitKaoshi();});
			...
			try{
				...
				&& !isNaN(parseInt(options['thisRecord'])))
				{
					try{
						...
						try{ShowKaoshi(thisRecord);}catch(err){}		
					}catch(err){}
		...
		*强制提交考试试卷
		*********************************************************************************************/
		var SubmitForceKaoshi = function(iClosed,back)
		{
			try{
				if(!Operate['ShowSubmitMasker']){alert('获取配置信息错误,请重试');return false;}
				if(!Operate['ShowSubmitContianer']){alert('获取配置信息错误,请重试');return false;}	
			}catch(err){}
			try{$(Operate['ShowSubmitMasker']).show();}catch(err){}
			var thisTimer = 8;
			var thisTimeout = setInterval(function(){
				try{
					...
					{
						try{
							thisTimer=thisTimer-1;
							$(Operate['ShowSubmitContianer']).html("考试答题时间已到,系统将在"+(thisTimer-2)+"后自动交卷");	
						}catch(err){}
					}else if(thisTimer>=1)
					{
						try{
							thisTimer=thisTimer-1;
							$(Operate['ShowSubmitContianer']).html("正在提交试卷,请等待...");
						}catch(err){}
					}
					else
					{
						try{clearInterval(thisTimeout);}catch(err){}
						try{SubmitKaoshi();}catch(err){}
					}
				}catch(err){}
			},1000);
		};
		...
		var MultipleC = function(iChecked)
		{
			...
			*****************************************************************************************/
			try{
				...
				****************************************************************************************/
				if(autoChecked.checked)
				{$("#frmList").find("label[id=\"labelC\"]").attr("selection","true");}
		...
		var VerificationSelectionMultiple = function(absText)
		{
			...
			*****************************************************************************************/
			try{
				...
				*开始遍历数据信息
				*****************************************************************************************/
				$("#frmList").find("input[operate=\"autoMul\"]").each(function(k,obj){
					if(absText.indexOf(this.value)!=-1){
						$(this).attr("ians","true");
						$(this.parentNode).attr("ians","true");
					}else{
						$(this).attr("ians","false");
						$(this.parentNode).attr("ians","false");	
					}
				});
			}catch(err){}
			/********************************************************************************************

	...
	$.fn.Kaoshi = function(options,arrList)
	{
		...
		*********************************************************************************************/
		var ShowKaoshi = function(SelectedIndex)
		{
			...
			*判断是否需要验证VIP会员信息
			*******************************************************************/
			if(SelectedIndex>=9 && options['isVip']!=undefined && options['isVip']!='True'){
				try{
					if($.messager!=undefined && $.messager!=null && typeof($.messager)=='object' 
					&& $.messager.confirm!=undefined && $.messager.confirm!=null 
					&& typeof($.messager.confirm)=='function'){
						$.messager.confirm('系统提示','你需要购买VIP会员才能继续答题,是否立即购买VIP？',function(isOK){
			...
			*******************************************************************/
			try{
				...
				else if(Operate['iSubmit']!=undefined && Operate['iSubmit']!=null 
				&& Operate['iSubmit']!="" && Operate['iSubmit']!='false')
				{
					$(Operate['FrmTrue']).attr("disabled","disabled");
					$(Operate['FrmFalse']).attr("disabled","disabled");
					$(Operate['FrmA']).attr("disabled","disabled");
					...
					for(var n in arrOptions)
					{
						...
						********************************************************************/
						...
						if(Operate['thisOption']['QuestionMode']=="多选")
						{
							...
							if(Operate['iSubmit']!=undefined && Operate['iSubmit']!=null 
							&& Operate['iSubmit']!="" && Operate['iSubmit']!='false')
							{strTemplate +=" disabled=\"disabled\" ";}
				...
				*****************************************************************************************/
				try{
					...
					&& Operate['thisOption']['abcText']!="")
					{
						...
						***********************************************************************************/
						try{
							if(arrList['items'][SelectedIndex]['iStored']!="1")
							{
								try{SaveFalse(Operate['thisOption']);}catch(err){}
								try{arrList['items'][SelectedIndex]['iStored']="1";}catch(err){}
							}
						}catch(err){}
		...
		*********************************************************************************************/
		try{
			...
			if(arrList!=undefined && arrList!=null && typeof(arrList)=='object' 
			&& arrList['items']!=undefined && arrList['items']!=null 
			&& arrList['items'].length>=1)
			{

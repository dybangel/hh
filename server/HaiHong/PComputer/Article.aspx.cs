...
{
    public partial class Article : Fooke.Web.BaseHelper
    {
        ...
        protected void strList()
        {
            ...
            if (Tab == null) { Response.Write(""); Response.End(); }
            else if (Tab.Rows.Count <= 0) { Response.Write(""); Response.End(); }
            /******************************************************************************************
             * 构建分页网页内容信息
             * ****************************************************************************************/
            StringBuilder strBuilder = new StringBuilder();
            strBuilder.AppendFormat("<table width=\"100%\" border=\"0\" cellpadding=\"0\" id=\"frm-input\" cellspacing=\"0\">");
            strBuilder.AppendFormat("<tr class=\"line\"><td colspan=\"3\"></td></tr>");
            foreach (DataRow Rs in Tab.Rows)
            {
                strBuilder.AppendFormat("<tr onclick=\"window.location='?action=details&classid={0}&ItemsID={1}'\" class=\"hback\">", Rs["classId"], Rs["ShowID"]);
                strBuilder.AppendFormat("<td class=\"spacing\">{0}</td>", Rs["title"]);
                strBuilder.AppendFormat("<td style=\"text-align:left\" class=\"name\">{0}</td>", Rs["title"]);

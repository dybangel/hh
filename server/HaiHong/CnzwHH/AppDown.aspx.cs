...
{
    public partial class AppDown : Fooke.Code.AdminHelper
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            switch (strRequest)
            {
                ...
                case "excel": this.VerificationRole("下载记录"); strExcel(); Response.End(); break;
                case "savexport": this.VerificationRole("任务列表"); SaveExport(); Response.End(); break;
                case "default": this.VerificationRole("下载记录"); strDefault(); Response.End(); break;
                case "computer": this.VerificationRole("数据统计"); strComputer(); Response.End(); break;
            }
        }
        ...
        protected void strExcel()
        {
            ...
            string strResponse = Master.Reader("template/appDown/excel.html");
        ...
        protected void strDefault()
        {
            ...
            strText.Append("<td class=\"Base\" colspan=\"10\">下载记录 >> 记录列表</td>");
            strText.Append("</tr>");
            strText.Append("<tr class=\"search\">");
            strText.Append("<td colspan=\"10\">");
            strText.Append("<form id=\"SearchForm\" action=\"AppDown.aspx\" method=\"get\">");
            ...
            strText.Append(FunctionBase.OptionList(new List<OptionMode>() {
                new OptionMode(){Value="youxiao",Text="已回调记录"},
                new OptionMode(){Value="wuxiao",Text="未回调记录"},
                new OptionMode(){Value="waiting",Text="等待完成"},
            ...
            strText.Append("<td width=\"100\">应用名称</td>");
            strText.Append("<td width=\"100\">下载用户</td>");
            strText.Append("<td width=\"80\">IP地址</td>");
            strText.Append("<td width=\"140\">所在城市</td>");
            strText.Append("<td width=\"200\">设备标识</td>");
            strText.Append("<td width=\"60\">获得奖励</td>");
            strText.Append("<td width=\"60\">是否回调</td>");
            strText.Append("<td width=\"60\">任务状态</td>");
            strText.Append("<td width=\"120\">下载日期</td>");
            strText.Append("</tr>");
            /***********************************************************************************************
             * 构建分页语句查询条件
             * **********************************************************************************************/
            ...
            if (isFast == "youxiao") { strParams += " and isCallback=1"; }
            else if (isFast == "wuxiao") { strParams += " and isCallback=0"; }
            else if (isFast == "waiting") { strParams += " and isFinish=0"; }
            else if (isFast == "complate") { strParams += " and isFinish=1"; }
            ...
            * **********************************************************************************************/
            ...
            PageCenterConfig.Columns = "TokenID,UserID,Nickname,AppID,Appname,DeviceType,DeviceCode,Amount,strIP,Cityname,isRound,isCallback,isBreak,isFinish,StarDate,LastDate";
            PageCenterConfig.Params = strParams;
            PageCenterConfig.Identify = "TokenID";
            PageCenterConfig.PageSize = PageSize;
            PageCenterConfig.Page = RequestHelper.GetPage();
            PageCenterConfig.Sort = " TokenID desc";
            PageCenterConfig.Tablename = "Fooke_AppDown";
            string strSQL = PageCenterHelper.MS2005(PageCenterConfig);
            int Record = PageCenterHelper.Record("Fooke_AppDown", strParams);
            ...
            * **********************************************************************************************/
            foreach (DataRow Rs in Tab.Rows)
            {
                ...
                strText.AppendFormat("<td>{0}</td>", Rs["Cityname"]);
                strText.AppendFormat("<td>{0}</td>", Rs["DeviceCode"]);
                strText.AppendFormat("<td style=\"color:#ff0000\">{0}元</td>", Rs["Amount"].ToString());
                strText.AppendFormat("<td>{0}</td>", (Rs["isCallback"].ToString() == "0" ? "<a class=\"vbtnRed\">未回调</a>" : "<a class=\"vbtn\">已回调</a>"));
                strText.AppendFormat("<td>{0}</td>", GetAffairsText(Rs["isFinish"].ToString()));
                strText.AppendFormat("<td>{0}</td>", new Fooke.Function.String(Rs["StarDate"].ToString()).cDate().ToString("yyyy-MM-dd HH:mm:ss"));
            ...
            * **********************************************************************************************/
            Fooke.SimpleMaster.SimpleMaster Master = new SimpleMaster.SimpleMaster();
            string strResponse = Master.Reader("template/appDown/default.html");
            strResponse = Master.Start(strResponse, new Fooke.SimpleMaster.Function((funName) =>
            {
                string strValue = string.Empty;
                switch (funName)
        ...
        protected void SaveExport()
        {
            ...
            * ****************************************************************************************/
            ...
            strTabs.Append("select " + strColumns + " from Fooke_AppDown where 1=1");
            ...
            if (StarDate.Length != 0 && StarDate.isDate()) { strTabs.Append(" and StarDate>='" + new Fooke.Function.String(StarDate).cDate().ToString("yyyy-MM-dd 00:00:00") + "'"); }
            if (EndDate.Length != 0 && EndDate.isDate()) { strTabs.Append(" and StarDate<='" + new Fooke.Function.String(EndDate).cDate().ToString("yyyy-MM-dd 23:59:59") + "'"); }
            /******************************************************************************************
            * 查询数据导出结果
            * ****************************************************************************************/
        ...
        {/**************************************************************************************
            ...
            * ************************************************************************************/
            ...
            else if (!EndDate.isDate()) { EndDate = DateTime.Now.ToString(); }
            /*****************************************************************************************
             * 数据统计信息
             * ****************************************************************************************/
            DataTable Tab = DbHelper.Connection.ExecuteFindTable("[Stored_FindAppDowmComputerTable]", new Dictionary<string, object>() {
             ...
             * *********************************************************************************************************/
            Fooke.SimpleMaster.SimpleMaster Master = new SimpleMaster.SimpleMaster();
            string strResponse = Master.Reader("template/appDown/computer.html");
            strResponse = Master.Start(strResponse, new Fooke.SimpleMaster.Function((funName) =>
            {
                string strValue = string.Empty;
                switch (funName)

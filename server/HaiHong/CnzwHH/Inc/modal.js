FookeModal = new Object();
FookeModal._variableName = 'FookeModal';
FookeModal.LoadingHtmlUrl = null;
FookeModal.WindowCssClasses = new Array();
FookeModal.WindowTitleCssClasses = new Array();
FookeModal.WindowCloseCssClasses = new Array();
FookeModal.WindowContentCssClasses = new Array();
FookeModal.WindowMaskCssClasses = new Array();
FookeModal.WindowFooterCssClasses = new Array();
FookeModal.WindowResizeCssClasses = new Array();
FookeModal.ZIndex = 100;
FookeModal._isShown = false;
FookeModal._initialized = false;
FookeModal._modal = null;
FookeModal._modalTitle = null;
FookeModal._modalClose = null;
FookeModal._modalAnimationMask = null;
FookeModal._modalMask = null;
FookeModal._modalIframe = null;
FookeModal._modalResize = null;
FookeModal._modalFooter = null;
FookeModal._modalContent = null;
FookeModal._animationHandle = null;
FookeModal._isOpening = false;
FookeModal._hiddenSelects = null;
FookeModal._checkForScrollResizeHandle = null;
FookeModal._lastModalInfo = null;
FookeModal._lastWindowInfo = null;
FookeModal._isDragging = false;
FookeModal._moveModalInfo = null;
FookeModal._resizeModalInfo = null;
FookeModal._isResizing = false;

FookeModal.Configure = function(loadingHtmlUrl, windowCssClasses, windowTitleCssClasses, windowCloseCssClasses, windowContentCssClasses, windowFooterCssClasses, windowResizeCssClasses, windowMaskCssClasses, zIndex)
{
	if(loadingHtmlUrl!=""){this.LoadingHtmlUrl = loadingHtmlUrl;}
...
FookeModal.IsShown = function()
{
	return this._isShown;
}

FookeModal._getWindowInfo = function()
{
...
FookeModal._getCurrentStyleValue = function(element, styleRule, jsStyleRule)
{
	var value = 0;
...
FookeModal._calculateStyleOffset = function(element)
{
...
FookeModal.Dialog = function (innerHtml,width, height,onCloseFunction, x, y, ignoreCloseAndAnimation)
{
	if(!width){width=450;}
	...
	else if (this._hiddenSelects)
	{
		for (var i = 0; i < this._hiddenSelects.length; i++)
		{
			if (this._hiddenSelects[i].Element.style.visibility == 'hidden')
				this._hiddenSelects[i].Element.style.visibility = this._hiddenSelects[this._hiddenSelects.length-1].Visibility;
		}
		this._hiddenSelects = null;
	}
	if (!this._initialized){this._initialize();}
	try{this._modalTitle.childNodes[1].innerHTML = "系统提示";}
	catch (err){}
	if (!ignoreCloseAndAnimation){
		this._modalContent.innerHTML = 	"<div class=\"FookeModalContentBox\">"+innerHtml+"</div>";
	}
	try
	{
		this._modalContent.innerHTML = 	"<div class=\"FookeModalContentBox\">"+innerHtml+"</div>"
	...
	else
	{
		...
		this._animate(0, -panelHeight, panelHeight / 3, .67);//控制拉伸长度
		this._animate(0, -panelHeight, panelHeight, .67);
	}
...
/**************************************************************************
直接在本页面显示内容,弹出Alert模拟层
***************************************************************************/

FookeModal.Alert = function (innerHtml,callback, width, height, onCloseFunction, x, y, ignoreCloseAndAnimation)
{
	if(!width){width=450;}
	if(!height){height=150;}
	if (!ignoreCloseAndAnimation && this._isShown){this.Close();}
	...
	try{this._modalTitle.childNodes[1].innerHTML = "提示";}
	catch (err){}
	if (!ignoreCloseAndAnimation){
		this._modalContent.innerHTML = 	"<div class=\"FookeModalContentBox\">"+innerHtml+"</div>";
	}
	try
	{
		this._modalContent.innerHTML = 	"<div class=\"FookeModalContentBox\">"+innerHtml+"</div>";
		try{
			var sureButton = document.createElement('input');
			...
			sureButton.value="确定";
			sureButton.className="button";
			this._modalFooter.appendChild(sureButton);
		}catch(err){}
		this._modalFooter.className="CommonModalFooterAlert";
		this._modalFooter.appendChild(sureButton);
	}
	catch (err){}
...
FookeModal.Confirm = function (innerHtml,callback,callback2, width, height, onCloseFunction, x, y, ignoreCloseAndAnimation)
{
	if(!width){width=450;}
	if(!height){height=150;}
	if (!ignoreCloseAndAnimation && this._isShown){this.Close();}
	...
	var FookeModalContentBoxHeight = parseInt(height-50)
	if (!ignoreCloseAndAnimation){
		this._modalContent.innerHTML = 	"<div style=\""+FookeModalContentBoxHeight+"px;\" class=\"FookeModalContentBox\">"+innerHtml+"</div>";
	}
	try
	{
		this._modalContent.innerHTML = 	"<div style=\""+FookeModalContentBoxHeight+"px;\" class=\"FookeModalContentBox\">"+innerHtml+"</div>";
...
FookeModal.Open = function (url, width, height, onCloseFunction, x, y, ignoreCloseAndAnimation)
{
	if (!ignoreCloseAndAnimation && this._isShown){this.Close();}
	else if (this._hiddenSelects)
	{
...
FookeModal._checkForScrollResize = function()
{
...
FookeModal.Close = function(returnValue)
{
	if (this._isShown)
	{
		if (!this._initialized)
...
FookeModal._initialize = function()
{
...
FookeModal._animate = function(targetValue, nextValue, step, acceleration)
{
	if (this._animationHandle)
		window.clearTimeout(this._animationHandle);
...
FookeModal._startDrag = function(event)
{
	if (!this._initialized)
		this._initialize();
...
FookeModal._endDrag = function(event)
{
	if (!this._initialized)
		this._initialize();
...
FookeModal._drag = function(event)
{
	if (!this._initialized)
		this._initialize();
...
FookeModal._startResize = function(event)
{
	if (!this._initialized)
		this._initialize();
...
FookeModal._endResize = function(event)
{
	if (!this._initialized)
		this._initialize();
...
FookeModal._resize = function(event)
{
	if (!this._initialized)
		this._initialize();
...
FookeModal.Configure('',['CommonModal'],['CommonModalTitle'],['CommonModalClose'],['CommonModalContent'],['CommonModalFooter'],['CommonModalResize'],['CommonModalMask'],100);
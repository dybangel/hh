...
{
    public partial class Excel : Fooke.Code.AdminHelper
    {
        /// <summary>
        /// 入口函数
        /// </summary>
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            switch (strRequest)
            {
                case "save": this.VerificationRole("导出数据"); SaveExcel(); Response.End(); break;
        ...
        protected void ListReport()
        {
            ...
            strText.Append(FunctionBase.OptionList(new List<OptionMode>() {
                new OptionMode(){Value="Appname",Text="搜渠道名称"}
            }, SearchType));
            ...
            strText.Append("<td width=\"100\">渠道</td>");
        ...
        protected void strDefault()
        {
            ...
            if (string.IsNullOrEmpty(EndDate)) { EndDate = DateTime.Now.AddDays(-0).ToString("yyyyMMdd"); }
            else if (EndDate.Length <= 0) { EndDate = DateTime.Now.AddDays(-0).ToString("yyyyMMdd"); }
            else if (EndDate.Length!=8) { EndDate = DateTime.Now.AddDays(-0).ToString("yyyyMMdd"); }
            /**************************************************************************************
             * 构建网页内容
             * *************************************************************************************/
            StringBuilder strText = new StringBuilder();
            ...
            strText.Append("<form id=\"iptforms\" action='?action=addlinksave' onsubmit=\"return _doPost(this)\" method='post'>");
            strText.Append("<tr class=\"xingmu\">");
            strText.Append("<td width=\"80\">渠道</td>");
            ...
            * **********************************************************************************************/
            ...
            strTabs.Append("(");
            strTabs.Append("    select Report.*,isNull(Chs.Appname,'未知应用') as Appname");
        ...
        protected void SaveExcel()
        {
            ...
            * 获取要导出数据的Sqltext
            * ********************************************************************************************************************/
            string exeText = RequestHelper.GetRequest("exeText").toString();
            if (string.IsNullOrEmpty(exeText)) { this.ErrorMessage("获取导出数据记录失败,请重试！"); Response.End(); }
            else if (exeText.Length <= 0) { this.ErrorMessage("获取导出数据记录失败,请重试！"); Response.End(); }
            /**********************************************************************************************************************
            * 将查询语句转换为标准的Sql执行语句
            * ********************************************************************************************************************/
            string executeSql = new Fooke.Function.String(exeText).ToEncryptionText().toString();
            if (string.IsNullOrEmpty(executeSql)) { this.ErrorMessage("获取数据查询语句失败,请重试！"); Response.End(); }
            else if (executeSql.Length <= 0) { this.ErrorMessage("获取数据查询语句失败,请重试！"); Response.End(); }
            else if (!executeSql.StartsWith("select")) { this.ErrorMessage("获取数据查询语句失败,请重试！"); Response.End(); }
            /**********************************************************************************************************************
             * 开始执行数据查询
             * ********************************************************************************************************************/
            DataTable thisTab = DbHelper.Connection.ExecuteDataTable(executeSql);
            if (thisTab == null) { this.ErrorMessage("没有需要导出的数据！"); Response.End(); }
            else if (thisTab.Rows.Count <= 0) { this.ErrorMessage("没有需要导出的数据！"); Response.End(); }
            /******************************************************************************************
            * 输出数据处理结果
            * ****************************************************************************************/

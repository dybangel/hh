...
namespace Fooke.Web.Admin
{
    public partial class Single : Fooke.Code.AdminHelper
    {
        ...
        protected void Add()
        {
            ...
            strText.Append("<option value=\"-1\">所有页面(不指定页面)</option>");
            strText.Append("<option value=\"-2\" style=\"color:#F00;\">当前页面通用</option>");
            strText.Append("<option value=\"0\">系统根目录</option>");
            strText.Append(SingleHelper.Options("0"));
            ...
            strText.Append(LabelHelper.SingleColumns());
            strText.Append("</td>");
            strText.Append("</tr>");
            string LabelContent = "<a href=\"{$linkurl/}\">{$title/}</a>";
            strText.Append("<tr class=\"hback\">");
        ...
        protected void Update()
        {
            ...
            strText.Append("<table width=\"99%\" border=\"0\" cellspacing=\"1\" cellpadding=\"3\" class=\"table\">");
            strText.Append("<tr class=\"change\">");
            strText.Append("<td class=\"Base\" colspan=\"4\">标签管理 >> 编辑标签 >> 单页列表</td>");
            strText.Append("</tr>");
            strText.Append("<tr class=\"hback\">");
            strText.Append("<td class=\"tips\">标签名称</td>");
            strText.Append("<td colspan=\"3\" class=\"check_box\">");
            ...
            strText.Append(SingleHelper.Options("0", ParentID));
            strText.Append("</select>");
            strText.Append("<span>请选择要调用输出的友情连接的栏目</span>");
            strText.Append("</td>");
            strText.Append("<td class=\"tips\">排序方式</td>");

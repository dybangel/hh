...
{
    public partial class DutyReport : Fooke.Code.AdminHelper
    {
        /// <summary>
        /// 入口函数
        /// </summary>
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            switch (strRequest)
            {
                ...
                case "list": this.VerificationRole("任务列表"); ListReport(); Response.End(); break;
        ...
        protected void ListReport()
        {
            ...
            * **********************************************************************************************/
            ...
            PageCenterConfig.Tablename = "Fooke_UserDutyReport";
            string strSQL = PageCenterHelper.MS2005(PageCenterConfig);
            int Record = PageCenterHelper.Record("Fooke_UserDutyReport", strParams);
            ...
            * **********************************************************************************************/
            foreach (DataRow Rs in Tab.Rows)
            {
                ...
                strText.AppendFormat("<td><a href=\"?action=list&UnionID={0}\">{1}</a></td>", Rs["UnionID"], Rs["strUnion"]);
            ...
            * **********************************************************************************************/
            ...
            string strResponse = Master.Reader("template/dutyReport/list.html");
        ...
        protected void strDefault()
        {
            ...
            * **********************************************************************************************/
            ...
            strTabs.Append("(");
            strTabs.Append("    select Report.*,isNull(Chs.strUnion,'自有渠道') as strUnion");
            strTabs.Append("    from GetDutyReport(" + StarDate + "," + EndDate + ") as Report");
            strTabs.Append("    left join Fooke_UnionChannel as Chs On Report.UnionID = Chs.UnionID");
            ...
            * **********************************************************************************************/
            ...
            PageCenterConfig.PageSize = PageSize;
            PageCenterConfig.Page = RequestHelper.GetPage();
            PageCenterConfig.Sort = " UnionID asc";
            ...
            * **********************************************************************************************/
            Fooke.SimpleMaster.SimpleMaster Master = new SimpleMaster.SimpleMaster();
            string strResponse = Master.Reader("template/dutyReport/default.html");
            strResponse = Master.Start(strResponse, new Fooke.SimpleMaster.Function((funName) =>
            {
                string strValue = string.Empty;
                switch (funName)
        ...
        protected void Delete()
        {
            ...
            * ********************************************************************************************************************/
             ...
             * 开始删除请求数据
             * ********************************************************************************************************************/
            try { DbHelper.Connection.Delete("Fooke_UserDutyReport", Params: strParamter); }
            catch { }
            /******************************************************************************************
             * 保存操作日志
             * ****************************************************************************************/
            try { SaveOperation("删除了任务数据报表ID(" + strParamter + ")"); }
            catch { }
            /**********************************************************************************************************************
             * 返回数据处理结果
             * ********************************************************************************************************************/

	...
	&& options['type']!=undefined && options['type']=='redirect'){
		try{
			if(options['url']!=undefined && options['url']!=''){
				if(options['url']!='reload'){window.location=options['url'];}
...
;(function($){
	...
	*加载倒计时
	*****************************************************************************/
	$.fn.interval=function(back,ShowDays){
		/*************************************************************************
		*获取到倒计时
		**************************************************************************/
		try{
		...
		try{
			try{
				var frmTemplate = "<font operate=\"days\">00</font> <font operate=\"hour\">00</font>:<font operate=\"minute\">00</font>:<font operate=\"second\">00</font>";
				$(thisT).html(frmTemplate);
			}catch(err){}
			var frmDays = $(thisT).find("font[operate=\"days\"]")[0];
			var frmHour = $(thisT).find("font[operate=\"hour\"]")[0];
			var frmMinute = $(thisT).find("font[operate=\"minute\"]")[0];
			var frmSecond = $(thisT).find("font[operate=\"second\"]")[0];
			try{
				if(!frmDays){frmDays=document.createElement('font');$(thisT).append(frmDays);}
				if(!frmHour){frmHour=document.createElement('font');$(thisT).append(frmHour);}
				if(!frmMinute){frmMinute=document.createElement('font');$(thisT).append(frmMinute);}
				if(!frmSecond){frmSecond=document.createElement('font');$(thisT).append(frmSecond);}
			}catch(err){}
			
			if(ShowDays!=undefined && !ShowDays){$(frmDays).hide();}
			if(interval<=0){
				if(back!=undefined && typeof(back)=='function'){
					back(0);
				};
				...
				try{
					var object = setInterval(function(){
						if(interval>0){
							...
							$(frmDays).html(days);
							$(frmHour).html(hour);
							$(frmMinute).html(Minute);
							$(frmSecond).html(Second);
						}
						else{

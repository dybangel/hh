...
$(function(){
	$("#frm-headerMenu").click(function(){
		if(document.querySelector("#frmCenterMaster"))
		{
			try{
				var showDisplay = $("#frmCenterMaster").attr("show") || "false";
				if(showDisplay!="true"){
					$(document.querySelector("#frmCenterMaster")).attr("show","false");
					$(document.querySelector("#frmCenterMaster")).show();	
				}else{
					$(document.querySelector("#frmCenterMaster")).attr("show","false");
					$(document.querySelector("#frmCenterMaster")).hide();
				}
			}catch(err){}
		}
		else{
			try{
				$.get("Index.aspx?action=menu",function(strResponse){
					if(strResponse!=undefined && strResponse!=null 
					&& strResponse!="" && strResponse!="error")
					{
						$(document.body).append(strResponse);
						if(document.querySelector("#frmCenterMaster") 
						&& $(strResponse)[0]!=undefined && $(strResponse)[0]!=null)
						{
							$(document.querySelector("#frmCenterMaster")).click(function(){
								$(document.querySelector("#frmCenterMaster")).hide();						
							});
							$(document.querySelector("#frmCenterMenu")).click(function(){
								return false;						
							});	
						}
					};
